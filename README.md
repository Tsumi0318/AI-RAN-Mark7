...
本仓库实现一个可复现的 AI-RAN 二元计算卸载实验。实验使用阿里巴巴公开的 GenTD26 匿名生产轨迹构造节点任务与中心资源状态，通过 DeepSeek 完成任务语义影响预测和中心 Game Master 协调，并由确定性 Python 公式核验中心代价、最佳响应和纯策略纳什均衡（PSNE）。
...
Mark7 根据多服务容器轨迹，将中心视为理想负载均衡的大算力池，并采用带固定开销与压力缩放的参数化形式：
...
因此它保留了 `1/(处理能力-负载)` 的非线性结构，但增加了固定开销 `D_overhead`、池化处理能力 `c*mu_card,eff` 和缩放参数 `a`。当 `D_overhead=0`、`a=1` 且 `mu=c*mu_card,eff` 时，该式退化为基础排队式。
...
拟合使用鲁棒对数残差最小二乘。`R-squared=0.02038` 表明该公式对当前网关时延散点的解释力很弱，不能宣称已经验证出一般拥塞规律，也不能把它当作高精度真实时延预测器。
...
逐任务预测保存在 `semantic_resource_predictions_reused.csv`，聚合校准参数保存在 `semantic_parameter_calibration.csv`。这种做法利用了任务语义信息，但会损失任务级异构性。
...
Python 始终重新计算上述结果。LLM 返回的数值只有在容差内与确定性结果一致时才被标记为一致，节点实际使用的是 Python 核验值，不是未经约束的自然语言输出。
...
随机基线的平均系统总代价 `183.93` 低于本次 PSNE 的 `208.10`。因此结果支持“协调策略达到 PSNE 并避免主实验容量违规”，但不支持“协调策略的系统总代价优于所有基准”，更不支持“达到全局最优”。
...
实测 API 时延约为秒级，不是微秒至毫秒级。因此当前结果不能证明 LLM 协调已经满足实时控制要求；它只说明该机制能够在离线软件仿真中完成语义反馈与数值核验。
